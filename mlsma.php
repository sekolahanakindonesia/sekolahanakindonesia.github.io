...
<body>
  ...
  <h2>Machine Learning - Prediksi Nilai Rapor SMA</h2>
  ...
  <script>
    const SHEET_URL = 'https://opensheet.elk.sh/1g-PQD3-zfCB6wugazPCPTVf2l3ecb9-AW2CKsGQx464/Sheet1';
    let dataMentah = [];
    ...
    function renderTabel() {
      ...
      dataMentah.forEach(row => {
        if ((kelas && row['KELAS'] !== kelas) || (mapel && row['MAPEL'] !== mapel)) return;
        if (materi && row['KETERANGAN MATERI'] !== materi) return;
        ...
        if (!kategoriList.includes(kategori)) return;
        ...
        if (isRaporMode) {
          ...
          for (const tp in nilaiTP) {
            for (const kategori of kategoriList) {
              ...
              if (!data) continue;
              const r = data.real.length ? data.real.reduce((a,b)=>a+b,0)/data.real.length : 0;
              const m = data.remedial.length ? data.remedial.reduce((a,b)=>a+b,0)/data.remedial.length : 0;
              const p = data.pengayaan.length ? data.pengayaan.reduce((a,b)=>a+b,0)/data.pengayaan.length : 0;
              const hasil = nilaiAkhir(r, m, p, kkm);
              if (hasil > 0) nilaiPerKategori[kategori].push(hasil);
            }
          }
          const nilaiRapor = kategoriList.map(k => nilaiPerKategori[k].length ? nilaiPerKategori[k].reduce((a,b)=>a+b,0)/nilaiPerKategori[k].length : 0);
          const raporFinal = nilaiRapor.filter(v => v > 0).length ? (nilaiRapor.reduce((a,b)=>a+b,0)/nilaiRapor.filter(v => v>0).length).toFixed(2) : '0';
          tbody.append(`<tr><td>${no++}</td><td>${key.split('|')[0]}</td><td class="text-left">${nama}</td><td>${kelas}</td><td>${kkm}</td><td class="text-left">${mapel}</td><td>${raporFinal}</td></tr>`);
        } else {
          for (const materi in nilaiMateri) {
            if ($('#materiFilter').val() && materi !== $('#materiFilter').val()) continue;
            const rowNilai = [];
            for (const kategori of kategoriList) {
              const d = nilaiMateri[materi][kategori] || { real: [], remedial: [], pengayaan: [] };
              const r = d.real.length ? (d.real.reduce((a,b)=>a+b,0)/d.real.length).toFixed(2) : '0';
const m = d.remedial.length ? (d.remedial.reduce((a,b)=>a+b,0)/d.remedial.length).toFixed(2) : '0';
const p = d.pengayaan.length ? (d.pengayaan.reduce((a,b)=>a+b,0)/d.pengayaan.length).toFixed(2) : '0';

const hasilAkhir = nilaiAkhir(r, m, p, kkm);
const hasilFormatted = hasilAkhir.toFixed(2);

const rClass = parseFloat(r) < kkm ? 'red-bold' : '';
const mClass = parseFloat(m) < kkm ? 'red-bold' : '';
const pClass = parseFloat(p) < kkm ? 'red-bold' : '';
const hasilClass = hasilAkhir < kkm ? 'red-bold' : '';

rowNilai.push(
  `<td class="${rClass}">${r}</td>` +
  `<td class="${mClass}">${m}</td>` +
  `<td class="${pClass}">${p}</td>` +
  `<td class="${hasilClass}">${hasilFormatted}</td>`
);

            }
            tbody.append(`<tr><td>${no++}</td><td>${key.split('|')[0]}</td><td class="text-left">${nama}</td><td>${kelas}</td><td>${kkm}</td><td class="text-left">${mapel}</td><td class="text-left">${materi}</td>${rowNilai.join('')}</tr>`);
          }
        }
      }
    }
